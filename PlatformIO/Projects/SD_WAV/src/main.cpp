...
//    I2S
          #define I2S_DOUT      33  //25          // i2S Data out oin
          #define I2S_BCLK      27  //27        // Bit clock
          #define I2S_LRC       15  //26        // Left/Right clock, also known as Frame clock or word select
...
void PlayWav()
{
  ...
  if(ReadingFile)                                     // Read next chunk of data in from file if needed
  {
    ...
    //finished = true;        // We keep calling this routine until it returns true, at which point
    //Serial.print("Finished File");
...
void changeWavFile(const char* file){

  WavFile = SD.open(file);                   // Open the wav file
  if(WavFile==false){
    //Serial.println("Could not open 'wavfile1.wav'");
...
{    
  ...
  if(millis() >= time_now + period){
    time_now += period;
    changeWavFile("/wavfile1.wav");
    Serial.println("Change true");
  }

  /*
  if (finished == true){
    //string file = ("wavfile1.wav")
    Serial.println("Finished true");
    changeWavFile("wavfile1.wav");
  }*/
